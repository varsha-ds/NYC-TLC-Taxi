...
This project builds a production-style demand forecasting pipeline for NYC TLC Yellow Taxi data. It ingests monthly trip records, cleans and types data into a processed layer, aggregates demand by pickup zone and hour, engineers time-series features (lags + rolling means), trains a model that outperforms strong baselines, generates predictions into Postgres, and evaluates backtests.
...
Output: predictions written to Postgres table predictions.yellow_demand_zone_hour
...
raw
Stores the ingested trip data as delivered (minimal transformation).

processed
Cleans + types raw columns into consistent, analysis-ready schema (timestamps, numeric types, ids).

marts
Business-ready aggregates:

marts.yellow_demand_zone_hour: hourly demand per pickup zone (trip_count) on a complete grid (zones × hours).

features
Model-ready features for each zone-hour:
...
predictions
Forecast outputs + metadata written by inference jobs:
...
This baseline is strong because taxi demand has strong daily seasonality.
...
appends prediction into the buffer
...
(model_name, train_end, horizon_hours, hour_start, pu_location)
Rerunning the same forecast overwrites those rows instead of creating duplicates.
...
(Optionally, --train_end can be omitted if your script auto-detects latest available cutoff.)
...
Timezone confusion (run_ts queries returning empty)

Prefer storing timestamps as timestamptz in UTC.

Avoid equality filters with microseconds; use time ranges or “latest run” selection.

Feature name/order mismatch at prediction time
...
Ensure SQL does not select a column twice (e.g., pu_location).

Duplicate logical forecasts

Use unique constraint on (model_name, train_end, horizon_hours, hour_start, pu_location)
